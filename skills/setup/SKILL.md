...
11b. Copies `skills/studio/` into the target (SKILL.md plus its launcher script, a few KB) — `/relay:studio` invokes `skills/studio/scripts/relay-studio.sh` by a project-relative path. The Studio app itself (`studio/`, a React/Vite tree) is deliberately **not** copied: the launcher locates it inside the Relay module and installs its dependencies there once, shared by every project.
...
- `/relay:studio` (see `skills/studio/SKILL.md`) gives an interactive editor for role prompts and for attaching/detaching `extraSkills` on any role by drag-and-drop, instead of hand-editing `.relay/agents.json`.
